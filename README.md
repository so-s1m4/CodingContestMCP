...
Team account pools are isolated by Telegram room. Set `BOT_SESSION_ENCRYPTION_KEY` to a persistent Fernet key (generate one with `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`) and set `MCP_PUBLIC_ORIGIN` to the public HTTPS origin. In the bot's private chat, use `/start` for the menu. Open a room once to get its own queue/history/disconnect panel; the selected room is saved and restored after redeploy. The bot accepts room passwords in a private message and deletes that message after receiving it. Account connections use a one-time HTTPS form for the CCC `SESSION` cookie; the server verifies the CCC identity and encrypts the session at rest. To select a pool, add `X-CCC-Team-Room: <room>` beside `X-CCC-Session` in the MCP connection headers. The submitting CCC account must itself be linked to that room. Fanout is queued only after CCC reports `evaluation.isCorrect: true`, and submissions are spaced by random 60–180 second intervals per target account. The queue shows each waiting account and its ETA; its button moves only that submission to the front. Recent completed submissions can be repeated for the same account from History; their answer files are retained for 30 days. Room data and sessions are stored in `CCC_DATA_DIR/telegram-pools.sqlite3`; the deployment must mount persistent storage at that path. Docker Compose uses a fixed named volume (`codingcontest-mcp-data`) for `/data`, so redeploys reuse the same data. Keep `BOT_SESSION_ENCRYPTION_KEY` unchanged across redeploys to decrypt saved sessions. The bot uses Telegram long polling, so only one running server instance may poll this bot token at a time.
